# This file reproduces the results for Lasso-SIR under model (M1) in Table 1 with p = 1000.
rm(list = ls())
library(pbmcapply)
library(MASS)
library(glmnet)
...
load("beta/beta1.rda")     # Load the beta matrix generated from "simulation1_dat.R"

## main program
foo <- function(i){
  cat("Time", i, '\n')
  load(paste0("dat/sim1/dat", i, ".rda"))       # Load the data set generated from "simulation1_dat.R"
  load(paste0("dat/sim1/foldid", i, ".rda"))    # Load the fold id generated from "simulation1_dat.R"
  x_train <- dat$x
  y_train <- dat$y
...
write.table(output, file = "output/M1_LassoSIR")
