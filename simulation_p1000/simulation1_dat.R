# This file generate data from model (M1) with p=1000.
rm(list = ls())
library(pbmcapply)
library(MASS)
source("utility.R")
...
beta[ind] <- runif(s, 0.1, 0.4)
save(beta, file = "beta/beta1.rda")  # Save beta matrix.
...
foo <- function(i){
  ...
  save(dat, file = paste0("dat/sim1/dat",i,'.rda'))   # Save data set.
  ...
  save(foldid, file = paste0("dat/sim1/foldid",i,'.rda'))    # Save the cross-validation fold id.
}
